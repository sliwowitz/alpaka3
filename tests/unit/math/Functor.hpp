...
namespace mathtest
{
    ...
    //! Specialization converting alpaka::math::Complex<T> to std::complex<T>
    template<typename T>
    struct StdLibType<alpaka::math::Complex<T>>
    {
...
#define ALPAKA_TEST_MATH_OP_FUNCTOR(NAME, ARITY, STD_OP, ALPAKA_OP, ...)                                              \
    ...
    using BinaryFunctorsReal = std::tuple<OpAtan2, OpCopysign, OpMax, OpMin, OpFmod, OpPow, OpRemainder>;
