...
function(append_recursive_relative_subdirs In_RootDir Out_RecursiveRelativeSubDirsVariableName)
    ...
    foreach(relativeFilePath IN LISTS recursiveRelativeFiles)
        get_filename_component(relativeSubDir "${relativeFilePath}" PATH)
        list(APPEND recursiveRelativeSubDirs "${relativeSubDir}")
    endforeach()
    #MESSAGE("recursiveRelativeSubDirs: ${recursiveRelativeSubDirs}")

    # If the list is not empty.
    list(LENGTH recursiveRelativeSubDirs recursiveRelativeSubDirsLength)
    if("${recursiveRelativeSubDirsLength}")
        # Remove duplicates from the list.
        list(REMOVE_DUPLICATES recursiveRelativeSubDirs)
        #MESSAGE("recursiveRelativeSubDirs: ${recursiveRelativeSubDirs}")

        # Set the return value (append it to the value in the parent scope).
        #MESSAGE("Out_RecursiveRelativeSubDirsVariableName: ${Out_RecursiveRelativeSubDirsVariableName}")
        set(${Out_RecursiveRelativeSubDirsVariableName}
...
function(add_recursive_files_to_src_group In_RootDir In_SrcGroupIgnorePrefix In_FileExtension)
    ...
    foreach(currentRelativeSubDir IN LISTS recursiveRelativeSubDirs ITEMS "")
        # Appended the current subdirectory.
        if(currentRelativeSubDir STREQUAL "")
            set(currentSubDir "${In_RootDir}")
        else()
            set(currentSubDir "${In_RootDir}/${currentRelativeSubDir}")
        endif()
        #MESSAGE("currentSubDir: ${currentSubDir}")
        # Get all the files in this sub-folder.
        set(wildcardFilePath "${currentSubDir}/*.${In_FileExtension}")
...
function(
    ...
    set(allFilePathsList "${${Out_FilePathsListVariableName}}")
...
function(list_add_prefix In_Prefix In_ListVariableName)
    ...
    foreach(item IN LISTS ${In_ListVariableName})
        if(NOT "${item}" STREQUAL "")
            list(APPEND local_list "${In_Prefix}${item}")
        endif()
    endforeach()

    set(${In_ListVariableName} "${local_list}" PARENT_SCOPE)
endfunction()
